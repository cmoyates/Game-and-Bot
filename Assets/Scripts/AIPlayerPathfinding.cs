...
public class AIPlayerPathfinding : MonoBehaviour
{
    class Node
    {
        // Info about the node
    ...
    void Start()
    {
        // Get references to the player, the players AI, the walls tilemap, and the dungeon generator
        player = GameObject.FindGameObjectWithTag("Player");
        ai = player.GetComponent<AIPlayerController>();
        ...
        // Set the goal to the players position (in case it tries to pathfind to something before the direction calcualtions have run)
    ...
    public void DirectionGridCalc()
    {
        ...
        // Get the bots current target's position on the grid
        int gx = Mathf.FloorToInt(ai.currentTargetPos.x) - offset.x;
        int gy = Mathf.FloorToInt(ai.currentTargetPos.y) - offset.y;
